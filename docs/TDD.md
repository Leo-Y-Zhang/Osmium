...
pre-run baseline, warm runs are served entirely from the free list, a
freed frame reads back zero through the physical alias before any
reallocation, and a frame soiled while sitting in the free list reads back
zero once handed out again (the two scrubs are proven separately, since
either alone would hide the other's deletion); the
...
| 10 | **`unsafe impl`** — a promise about a whole type, not one expression | `memory`, `memory::frames`, `memory::heap`, `gdt` | Five of them, each carrying its `// SAFETY:` comment on the impl itself, since there is no block to attach one to. **`FrameAllocator for BootFrameAllocator`** promises a frame is never handed out twice **at a time** — the wording M11 forced, because a reclaimed frame is now legitimately handed out again. Two arguments hold it up: the bump half is unchanged (`next` only ever grows and indexes a deterministic iterator over a boot memory map that does not change after hand-over, so frame *n* leaves the cursor exactly once), and the reuse half rests on `deallocate_frame`, the only way into the free list, which refuses a frame already in it and a frame the cursor never issued — both real `assert!`s. What no assert can see is a free of a frame still mapped somewhere; that is excluded by construction instead, since the sole caller is `AddressSpace::drop` returning exactly the frames that space recorded pulling. **`FrameAllocator for RecordingAllocator`** (M11) adds nothing to that contract: it delegates every hand-out to `BootFrameAllocator` and only records the frame against the space that pulled it, and recording does not change ownership. It exists so that frames the mapping machinery allocates internally — intermediate page tables no call site ever sees — are still owned by the space that caused them. **`GlobalAlloc for ZeroOnFree`** adds nothing to `LockedHeap`'s contract and defers to it for both calls; the only extra act is scrubbing a block the caller has already relinquished, in the window between the caller's last legal access and the inner `dealloc` — after that `dealloc` the allocator owns those bytes, which is why the order is fixed (row 7). **`Sync for IstStack`** covers the three IST `UnsafeCell`s of row 6: nothing in the kernel reads or writes that memory, so there is no cross-thread access to make sound; only the CPU touches it, and only while handling the fault the slot is wired to. **`Sync for PrivilegeStack`** is the same promise for the TSS RSP0 stack: only the CPU writes it, on a privilege transition into the kernel. |
...
- *Mutation, M11 adversarial-review round (observed failing 2026-08-18):*
  ...
  free list carried its bytes"; the reclamation and free-time-scrub tests
  both still pass, which is exactly why the probe had to exist). The review
  found that line unfalsifiable as first written: `soil_next_frame`'s
  free-list branch never ran, because its only caller sits earlier in the
  battery than the first `deallocate_frame`, so the free list was provably
  empty at soil time and the popped frame read zero courtesy of the
  free-time scrub. The probe closes it by soiling the frame AFTER freeing
  it — the one ordering only the hand-out scrub can clean up. **A scrub on
  a path no test can reach is decoration, and the second scrub in a
  belt-and-braces pair is exactly where that hides.**
