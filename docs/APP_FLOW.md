...
| `mem` | Heap used and free against its 1 MiB size; frames in use of the usable total, with usable RAM in MiB and the running total of reclaimed frames (M11) — run `user` or `crash`, then `mem` again, and the in-use count is back where it was while the reclaimed count has grown. | None. Reads the heap's own counters and, behind the frame allocator's lock, its cursor and free-list length; the usable-frames figure is a walk of the boot memory map, done twice per invocation and never in interrupt context. |
...
| `ls` | Every file in the RAM-only filesystem (M12) with its size, then a footer giving the file count, bytes used of the arena total, and the reminder that all of it is in RAM and none of it on disk. | None. An empty filesystem prints `no files` and the footer, which is a statement rather than a blank. |
| `write <name> <text>` | Creates a file whose contents are the rest of the line, and confirms the name and byte count. **It never overwrites** — an existing name must be deleted first, so a mistyped name cannot silently destroy a file. | Every refusal names the rule that fired, as `error: <sentence>`: a missing name, one over 32 bytes, one containing a space or path separator (the namespace is flat and has no traversal to refuse), a duplicate, no free slots, or not enough arena left. **Every one of those is reachable from this prompt** — the arena is deliberately smaller than eight full command lines, so `not enough space left in the filesystem` is a refusal a person can actually provoke rather than one only a host test ever sees. |
| `cat <name>` | Prints a file's contents to the console. Like all command output it never reaches the serial port — which `cargo xtask privacy` proves by typing the sentinel into a file and `cat`ing it back with serial required to stay silent. | `error: no such file`. A file whose bytes are not valid UTF-8 is reported rather than printed, so `cat` cannot be made to panic by a future writer of arbitrary bytes. |
| `rm <name>` | Deletes a file and says so, noting that its bytes were scrubbed — deletion zeroes the contents and compacts the arena, so the space is genuinely reusable and nothing lingers. | `error: no such file`. |
